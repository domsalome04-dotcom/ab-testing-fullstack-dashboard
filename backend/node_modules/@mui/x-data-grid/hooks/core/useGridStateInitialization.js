...
const useGridStateInitialization = apiRef => {
  ...
  const setState = React.useCallback((state, reason) => {
    ...
    if (updatedControlStateIds.length > 1) {
      ...
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: You're not allowed to update several sub-state in one transaction. You already updated ${updatedControlStateIds[0].stateId}, therefore, you're not allowed to update ${updatedControlStateIds.map(el => el.stateId).join(', ')} in the same transaction.` : (0, _formatErrorMessage2.default)(66, updatedControlStateIds[0].stateId, updatedControlStateIds.map(el => el.stateId).join(', ')));
    }
    if (!ignoreSetState) {
