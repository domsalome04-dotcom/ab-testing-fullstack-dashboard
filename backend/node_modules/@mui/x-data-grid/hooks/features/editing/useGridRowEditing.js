...
const useGridRowEditing = (apiRef, props) => {
  ...
  const throwIfNotInMode = React.useCallback((id, mode) => {
    if (apiRef.current.getRowMode(id) !== mode) {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X Data Grid: The row with id=${id} is not in ${mode} mode. ` + 'The operation requires the row to be in a specific editing mode. ' + `Ensure the row is in ${mode} mode before performing this operation.` : (0, _formatErrorMessage2.default)(79, id, mode, mode));
    }
  }, [apiRef]);
