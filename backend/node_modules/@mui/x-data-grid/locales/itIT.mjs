import { getGridLocalization, buildLocaleFormat } from "../utils/getGridLocalization.mjs";
...
const itITGrid = {
  ...
  paginationDisplayedRows: ({
    ...
    if (!estimated) {
      return `${formatNumber(from)}–${formatNumber(to)} di ${count !== -1 ? formatNumber(count) : `più di ${formatNumber(to)}`}`;
    }
    const estimatedLabel = estimated && estimated > to ? `circa ${formatNumber(estimated)}` : `più di ${formatNumber(to)}`;
    return `${formatNumber(from)}–${formatNumber(to)} di ${count !== -1 ? formatNumber(count) : estimatedLabel}`;
  },
