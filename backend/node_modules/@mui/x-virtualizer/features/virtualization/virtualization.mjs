...
import { nextAnchorTop } from "./anchor.mjs";
...
const selectors = (() => {
  ...
  return {
    ...
    anchorTop: createSelector(state => state.virtualization.anchorTop),
...
function useVirtualization(store, params, api) {
  ...
  const scrollCache = useLazyRef(() => createScrollCache(isRtl, rowBufferPx, columnBufferPx, averageRowHeight(store, rowHeight) * 15, MINIMUM_COLUMN_WIDTH * 6, layoutMode)).current;
  ...
  const updateRenderContext = React.useCallback((nextRenderContext, nextAnchor) => {
    ...
    if (!areRenderContextsEqual(nextRenderContext, state.renderContext) || anchorTop !== state.anchorTop) {
      store.set('virtualization', _extends({}, state, {
  ...
  const triggerUpdateRenderContext = useEventCallback((isSettlePass = false) => {
...
function createScrollCache(isRtl, rowBufferPx, columnBufferPx, verticalBuffer, horizontalBuffer, layoutMode = 'uncontrolled') {
  return {
    ...
    buffer: bufferForDirection(isRtl, ScrollDirection.NONE, rowBufferPx, columnBufferPx, verticalBuffer, horizontalBuffer, layoutMode)
  };
}
