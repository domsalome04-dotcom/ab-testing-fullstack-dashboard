...
const SCROLL_DELAY_LEVELS = [{
  minVelocityPxPerMs: 28,
  frames: 6
}, {
  minVelocityPxPerMs: 20,
  frames: 4
}, {
  minVelocityPxPerMs: 16,
  frames: 2
}, {
  minVelocityPxPerMs: 8,
  frames: 1
}];
function scrollDelayFrames(velocityPxPerMs) {
...
const selectors = (() => {
  ...
  return {
    ...
    anchorTop: (0, _store.createSelector)(state => state.virtualization.anchorTop),
...
function initializeState(params) {
  ...
  // re-render.
  const renderContext = enabled === false && enabledForRows === false && enabledForColumns === false ? {
    firstRowIndex: 0,
  ...
  const state = {
    virtualization: (0, _extends2.default)({
      enabled: !_platform.platform.env.jsdom,
      enabledForRows: !_platform.platform.env.jsdom,
      enabledForColumns: !_platform.platform.env.jsdom,
      renderContext,
...
function useVirtualization(store, params, api) {
  ...
  const layoutMode = (0, _store.useStore)(store, selectors.layoutMode);
  ...
  const scrollCache = (0, _useLazyRef.default)(() => createScrollCache(isRtl, rowBufferPx, columnBufferPx, averageRowHeight(store, rowHeight) * 15, MINIMUM_COLUMN_WIDTH * 6, layoutMode)).current;
  ...
  const updateScrollCacheBuffer = direction => {
    ...
    scrollCache.buffer = bufferForDirection(isRtl, direction, rowBufferPx, columnBufferPx, averageRowHeight(store, rowHeight) * 15, MINIMUM_COLUMN_WIDTH * 6, layoutMode);
  };
  const updateRenderContext = React.useCallback((nextRenderContext, nextAnchor) => {
    ...
    const anchorTop = nextAnchor ?? anchorTopFor(store, state.layoutMode, nextRenderContext, false);
    if (!areRenderContextsEqual(nextRenderContext, state.renderContext) || anchorTop !== state.anchorTop) {
      store.set('virtualization', (0, _extends2.default)({}, state, {
  ...
  const triggerUpdateRenderContext = (0, _useEventCallback.default)((isSettlePass = false) => {
    ...
    const rowVelocity = dtSinceLastScroll > 0 && dtSinceLastScroll < MAX_VELOCITY_SAMPLE_GAP_MS ? Math.abs(dy) / dtSinceLastScroll : 0;
    ...
    const isSettled = isSettlePass && direction === _models.ScrollDirection.NONE;
    let shouldUpdate;
    if (layoutMode === 'sticky') {
      ...
      const didCrossThreshold = rowScroll >= averageRowHeight(store, rowHeight) || columnScroll >= MINIMUM_COLUMN_WIDTH;
    ...
    if (layoutMode === 'sticky' && !forceStickyCommit.current && !didChangeDirection && !isSettlePass && (isDeferralPending || scrollDelayFrames(rowVelocity) > 0)) {
    ...
    const inputs = inputsSelector(store, params, api, enabledForRows, enabledForColumns, layoutMode);
  ...
  const getRows = (rowParams = {}, unstable_rowTree) => {
    ...
    rowIndexes.forEach(rowIndexInPage => {
      ...
      const offsetLeft = computeOffsetLeft(columnPositions, currentRenderContext, pinnedColumns.left.length, layoutMode);
...
function inputsSelector(store, params, api, enabledForRows, enabledForColumns, layoutMode) {
  const dimensions = _dimensions.Dimensions.selectors.dimensions(store.state);
...
function computeRenderContext(inputs, scrollPosition, scrollCache) {
  ...
  if (inputs.enabledForColumns) {
    ...
    if (!hasRowWithAutoHeight || inputs.virtualizeColumnsWithAutoRowHeight) {
      ...
      const rightBound = inputs.layoutMode === 'controlled' ? realLeft + inputs.viewportInnerWidth - inputs.rightPinnedWidth : realLeft + inputs.viewportInnerWidth;
 ...
 */
function anchorTopFor(store, layoutMode, renderContext, isSettled) {
  const {
    anchorTop
  ...
  const rowPositions = _dimensions.Dimensions.selectors.rowPositions(store.state);
  return (0, _anchor.nextAnchorTop)(anchorTop, rowPositions[renderContext.firstRowIndex] ?? 0, isSettled);
}
 ...
 */
function averageRowHeight(store, rowHeight) {
  const rowsMeta = _dimensions.Dimensions.selectors.rowsMeta(store.state);
...
function bufferForDirection(isRtl, direction, rowBufferPx, columnBufferPx, verticalBuffer, horizontalBuffer, layoutMode = 'uncontrolled') {
  if (layoutMode === 'controlled') {
    return EMPTY_BUFFER;
  }
  if (isRtl) {
 ...
 */
function isLowOnRenderedBuffer(store, params, scrollPosition, scrollCache) {
  ...
  const dimensions = _dimensions.Dimensions.selectors.dimensions(store.state);
  const rowPositions = _dimensions.Dimensions.selectors.rowPositions(store.state);
  const contentHeight = _dimensions.Dimensions.selectors.contentHeight(store.state);
  ...
  if (context.lastRowIndex < params.rows.length && renderedBottom - visibleBottom < buffer.rowAfter / 2) {
    return true;
  }
  const columnPositions = _dimensions.Dimensions.selectors.columnPositions(store.state, params.columns);
  ...
  if (context.firstColumnIndex > pinnedLeftCount && visibleLeft - renderedLeft < buffer.columnBefore / 2) {
    return true;
  }
  if (context.lastColumnIndex < params.columns.length - pinnedRightCount && renderedRight - visibleRight < buffer.columnAfter / 2) {
    return true;
  }
  return false;
}
function createScrollCache(isRtl, rowBufferPx, columnBufferPx, verticalBuffer, horizontalBuffer, layoutMode = 'uncontrolled') {
  return {
    direction: _models.ScrollDirection.NONE,
    buffer: bufferForDirection(isRtl, _models.ScrollDirection.NONE, rowBufferPx, columnBufferPx, verticalBuffer, horizontalBuffer, layoutMode)
  };
}
